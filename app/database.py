...
async def add_ban(name: str, steamid: str, reason: str):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("INSERT INTO bans (name, steamid, reason) VALUES (?, ?, ?)", (name, steamid, reason))
        await db.commit()
    
async def remove_ban(steamid: str) -> bool:
    async with aiosqlite.connect(DB_FILE) as db:
        cursor = await db.execute("DELETE FROM bans WHERE steamid = ?", (steamid,))
...
async def get_ban():
    async with aiosqlite.connect(DB_FILE) as db:
        cursor = await db.execute("SELECT name, steamid, reason FROM bans")
        rows = await cursor.fetchall()
        return [{"name": row[0], "id": row[1], "reason": row[2]} for row in rows]

async def get_ban_name(name: str):
    async with aiosqlite.connect(DB_FILE) as db:
        cursor = await db.execute("SELECT name, steamid, reason FROM bans WHERE name LIKE ?", (f"%{name}%",))
        rows = await cursor.fetchall()
        return [{"name": row[0], "id": row[1], "reason": row[2]} for row in rows]
